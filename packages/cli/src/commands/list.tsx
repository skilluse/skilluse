  ...
  useEffect(() => {
    async function loadSkills() {
      ...
      for (let i = 0; i < skills.length; i++) {
        setState({ phase: "checking_updates", current: i + 1, total: skills.length });
      ...
      setState({ phase: "success", skills: skillsWithUpdates, showingOutdated: true });
      exit();
    }
      ...
      if (state.showingOutdated) {
        return (
          <Box flexDirection="column">
            ...
            <Box marginTop={1}>
              <Text dimColor>
                Run 'skilluse upgrade' to update all, or 'skilluse upgrade skill-name' for one.
              </Text>
            </Box>
          </Box>
        );
