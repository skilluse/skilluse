...
export const args = z.tuple([
  z.string().optional().describe("Skill name to upgrade (optional, upgrades all if omitted)"),
]);
...
type UpgradeState =
  ...
  | { phase: "no_updates" }
  | { phase: "upgrading"; upgrades: UpgradeInfo[]; current: number; progress: number }
  ...
  useEffect(() => {
    async function upgrade() {
      ...
      for (let i = 0; i < skillsToCheck.length; i++) {
        setState({ phase: "checking_updates", current: i + 1, total: skillsToCheck.length });

        const skill = skillsToCheck[i];
        const repoConfig = config.repos.find((r) => r.repo === skill.repo);
        if (!repoConfig) continue;

        const update = await checkForUpdate(credentials.token, skill, repoConfig);
        if (update) {
