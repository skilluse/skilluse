...
import { Heading, type HeadingProps } from "~/components/shared/heading"
...
const IntroDescription = ({ className, ...props }: ComponentProps<"h2">) => {
  return (
    <h2
      className={cx(
        "max-w-2xl text-pretty text-muted-foreground [word-break:break-word] md:text-lg *:[&[href]]:underline *:[&[href]]:hover:text-primary",
        className,
      )}
      {...props}
    />
